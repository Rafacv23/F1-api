...
test.describe("GET /api/drivers/[driverId]", async () => {
  test("should return driver data for a valid teamId", async ({ request }) => {
    const response = await request.get(`/api/drivers/alonso`)
  ...
  test("should return 404 when no driver are found", async ({ request }) => {
    const response = await request.get(`/api/drivers/inventedDriver`)
