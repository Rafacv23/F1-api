...
test.describe("GET /api/[year]/teams/[teamId]/drivers", async () => {
  test("should return drivers data for a valid teamId and year", async ({
    request,
  }) => {
    const response = await request.get(`/api/2024/teams/ferrari/drivers`)
  ...
  test("should return 404 when no team are found", async ({ request }) => {
    const response = await request.get(`/api/2024/teams/inventedTeam/drivers`)
  ...
  test("should return 404 when no year are found", async ({ request }) => {
    const response = await request.get(
      `/api/inventedYear/teams/ferrari/drivers`
    )
    expect(response.status()).toBe(404)
