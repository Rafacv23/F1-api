...
test.describe("GET /api/[year]/teams/[teamId]", async () => {
  test("should return team data for a valid teamId and year", async ({
    request,
  }) => {
    const response = await request.get(`/api/2024/teams/ferrari`)
  ...
  test("should return 404 when no team are found", async ({ request }) => {
    const response = await request.get(`/api/2024/teams/inventedTeam`)
    expect(response.status()).toBe(404)
