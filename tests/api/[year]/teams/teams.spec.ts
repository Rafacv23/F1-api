...
test.describe("GET /api/[year]/teams", async () => {
  test("should return teams data with default limit and offset and a valid year", async ({
    request,
  }) => {
    const response = await request.get(`/api/2024/teams`)
  ...
  test("should respect limit and offset query parameters", async ({
    ...
    const response = await request.get(
      `/api/2024/teams?limit=${limit}&offset=${offset}`
    )
    expect(response.ok()).toBeTruthy()
    expect(response.status()).toBe(200)
