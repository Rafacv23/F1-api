...
test.describe("GET /api/[year]/[round]/fp2", async () => {
  test("should return fp2 race data for a valid year and round, with default limit and offset", async ({
    request,
  }) => {
    const response = await request.get(`/api/2024/3/fp2`)
  ...
  test("should return fp2 race data for a valid year and round, with limit and offset", async ({
    request,
  }) => {
    const response = await request.get(`/api/2024/3/fp2?limit=1&offset=1`)
    expect(response.ok()).toBeTruthy()
    expect(response.status()).toBe(200)
  ...
  test("should return 404 when no round are found", async ({ request }) => {
    const response = await request.get(`/api/2024/30/fp2`)
    expect(response.status()).toBe(404)
