...
test.describe("GET /api/[year]/[round]/sprint/qualy", async () => {
  test("should return sprint qualy race data for a valid year and round, with default limit and offset", async ({
    request,
  }) => {
    const response = await request.get(`/api/2024/5/sprint/qualy`)
  ...
  test("should return sprint qualy race data for a valid year and round, with limit and offset", async ({
    request,
  }) => {
    const response = await request.get(
      `/api/2024/5/sprint/qualy?limit=1&offset=1`
    )
    expect(response.ok()).toBeTruthy()
    expect(response.status()).toBe(200)
  ...
  test("should return 404 when no sprint qualy are found for this round", async ({
    ...
    const response = await request.get(`/api/2024/1/sprint/qualy`)
    expect(response.status()).toBe(404)
