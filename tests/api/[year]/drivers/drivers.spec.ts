...
test.describe("GET /api/[year]/drivers", async () => {
  test("should return drivers data with default limit and offset for a valid year", async ({
    request,
  }) => {
    const response = await request.get(`/api/2024/drivers`)
  ...
  test("should respect limit and offset query parameters", async ({
    ...
    const response = await request.get(
      `/api/2024/drivers?limit=${limit}&offset=${offset}`
    )
    expect(response.ok()).toBeTruthy()
    expect(response.status()).toBe(200)
