...
test.describe("GET /api/[year]/drivers/[driverId]", async () => {
  test("should return driver data for a valid driverId and year", async ({
    request,
  }) => {
    const response = await request.get(`/api/2024/drivers/alonso`)
  ...
  test("should return 404 when no driver are found", async ({ request }) => {
    const response = await request.get(`/api/2024/drivers/inventedDriver`)
