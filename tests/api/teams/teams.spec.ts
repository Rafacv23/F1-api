...
test.describe("GET /api/teams", async () => {
  test("should return teams data with default limit and offset", async ({
    request,
  }) => {
  ...
  test("should respect limit and offset query parameters", async ({
    ...
    const response = await request.get(
      `/api/teams?limit=${limit}&offset=${offset}`
    )
    expect(response.ok()).toBeTruthy()
    expect(response.status()).toBe(200)
  ...
  test("should return 404 when no teams are found", async ({ request }) => {
    const response = await request.get(`/api/teams?offset=1000`)
