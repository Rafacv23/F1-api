...
test.describe("GET /api/teams/[teamId]", async () => {
  test("should return team data for a valid teamId", async ({ request }) => {
    const response = await request.get(`/api/teams/adams`)
  ...
  test("should return 404 when no team are found", async ({ request }) => {
    const response = await request.get(`/api/teams/inventedTeam`)
