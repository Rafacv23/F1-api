...
test.describe("GET /api/current/drivers", async () => {
  test("should return drivers data with default limit and offset for and the current year", async ({
    request,
  }) => {
    const response = await request.get(`/api/current/drivers`)
  ...
  test("should respect limit and offset query parameters", async ({
    ...
    const response = await request.get(
      `/api/current/drivers?limit=${limit}&offset=${offset}`
    )
    expect(response.ok()).toBeTruthy()
    expect(response.status()).toBe(200)
  ...
  test("should return 404 when no drivers are found", async ({ request }) => {
    const response = await request.get(`/api/inventedYear/drivers`)
